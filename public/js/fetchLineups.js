 ...
 */
export async function fetchAndRenderLineups(lineupContainer) {
    try {
   ...
   *    and by show (using entry.show_name).
  ...
  export function renderLineups(lineupContainer, showLineups) {
    ...
    if (Array.isArray(showLineups) && showLineups.length > 0) {
      ...
      showLineups.forEach((entry) => {
        ...
        const show = entry.show_name || "Unknown Show";
        if (!grouped[category]) {
          grouped[category] = {};
        }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const sampleData = [
      { category_name: "Lop", show_name: "Show A", breed_name: "American Lop" },
      { category_name: "Lop", show_name: "Show A", breed_name: "Dutch Lop" },
      { category_name: "Lop", show_name: "Show B", breed_name: "English Lop" }
    ];
  ...
  */
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    fetchAndRenderLineups(container);
  });
